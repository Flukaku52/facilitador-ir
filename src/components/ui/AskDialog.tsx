...
export default function AskDialog({ profile }: AskDialogProps) {
 ...
 {/* Header */}
 <div className="flex items-center justify-between border-b border-border/50 px-5 py-4 dark:border-border">
 <div>
 ...
 <button
 onClick={handleClose}
 className="rounded-md p-1.5 text-muted hover:bg-slate-100 hover:text-body dark:hover:bg-slate-800 dark:hover:text-foreground transition-colors"
 aria-label="Fechar"
 >
 ✕
 </button>
 ...
 className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-primary-400 focus:outline-none focus:ring-1 focus:ring-primary-300 dark:bg-slate-800 dark:text-foreground dark:placeholder-muted resize-none"
 disabled={loading}
 />
 <span className={`absolute bottom-2 right-2 text-xs ${question.length >= MAX_QUESTION_LENGTH ? 'text-danger-500' : 'text-muted'}`}>
 ...
 {/* Answer */}
 {answer && (
 <div className="rounded-lg border border-border bg-background px-4 py-3 dark:bg-slate-800 max-h-64 overflow-y-auto">
