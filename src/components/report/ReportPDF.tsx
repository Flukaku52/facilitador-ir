...
const ALERT_CONFIG: Record<AlertSeverity, { bg: string; border: string; sevColor: string; label: string }> = {
  danger: { bg: '#FEE2E2', border: '#DC2626', sevColor: '#991B1B', label: 'CRÍTICO' },
  warning: { bg: '#FEF3C7', border: '#D97706', sevColor: '#92400E', label: 'ATENÇÃO' },
  info: { bg: '#EFF6FF', border: '#3B82F6', sevColor: '#1E40AF', label: 'INFO' },
};
...
const S = StyleSheet.create({
  ...
  listMark: { fontSize: 9, width: 26, flexShrink: 0, color: '#9CA3AF' },
  listText: { fontSize: 9.5, flex: 1, color: '#374151' },
  listTextDone: { fontSize: 9.5, flex: 1, color: '#9CA3AF', textDecoration: 'line-through' },
  // Pending box
  pendingBox: { backgroundColor: '#FEF3C7', borderRadius: 4, padding: 10 },
  pendingRow: { flexDirection: 'row', marginBottom: 3, alignItems: 'flex-start' },
  pendingBullet: { fontSize: 9.5, color: '#92400E', width: 12, flexShrink: 0 },
  pendingText: { fontSize: 9.5, color: '#92400E', flex: 1 },
  // Alert
  alertBox: { padding: 9, marginBottom: 7, borderRadius: 4, borderLeftWidth: 3 },
  ...
  alertMsg: { fontSize: 9, color: '#374151' },
  // Guides
  guideItem: { fontSize: 9.5, color: '#374151', marginBottom: 3 },
  // Next steps
  ...
  stepText: { fontSize: 9.5, color: '#374151', flex: 1 },
  // Legal disclaimer
  disclaimer: {
    marginTop: 16,
    padding: 10,
  ...
  disclaimerText: { fontSize: 8, color: '#6B7280', lineHeight: 1.5 },
  // Fixed footer
  footer: {
    position: 'absolute',
    bottom: 24,
...
// ---------------------------------------------------------------------------
export default function ReportPDF({ profile, complexity, checklist, guides, alerts }: Props) {
      ...
      <Page size="A4" style={S.page}>
          ...
          <View style={S.section}>
            ...
            <View style={S.grid}>
              {profileItems.map((item) => (
                <Text key={item} style={S.gridCell}>• {item}</Text>
              ))}
            </View>
          </View>
        )}
          ...
          <View style={S.section}>
            ...
            {checklist.map((item) => (
              <View key={item.id} style={S.listRow}>
                <Text style={S.listMark}>{item.completed ? '[ok]' : '[  ]'}</Text>
          ...
          <View style={S.section}>
            ...
            {alerts.map((alert) => {
              ...
              return (
                <View
                  key={alert.id}
                  style={{ ...S.alertBox, backgroundColor: cfg.bg, borderLeftColor: cfg.border }}
                >
                  <Text style={{ ...S.alertSev, color: cfg.sevColor }}>{cfg.label}</Text>
        ...
        {guides.length > 0 && (
          <View style={S.section}>
            ...
            {guides.map((guide) => (
              <Text key={guide.slug} style={S.guideItem}>• {guide.title}</Text>
            ))}
          </View>
        )}
        ...
        <View style={S.disclaimer}>
          <Text style={S.disclaimerText}>
            AVISO LEGAL: Este relatório fornece orientação educacional e organizacional para
            facilitar o preenchimento da declaração. Ele não substitui contador, advogado, a
            Receita Federal ou orientação profissional. A responsabilidade final pelas informações
            declaradas é do contribuinte.
          </Text>
        </View>
